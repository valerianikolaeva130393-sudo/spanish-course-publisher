...
try {
  ...
  const reviewPolls = lessons.find(item => Array.isArray(item.monthly_review_polls))?.monthly_review_polls;
  if (!Array.isArray(reviewPolls) || reviewPolls.length < 3 || reviewPolls.length > 7) {
    errors.push("Итоговый опросник месяца должен содержать от 3 до 7 Poll");
  } else {
    reviewPolls.forEach((poll, index) => {
      if (!poll?.question || !Array.isArray(poll.options) || poll.options.length < 2) {
        errors.push(`Некорректный итоговый Poll ${index + 1}`);
      }
      if (
        !Number.isInteger(poll?.correct_option_id) ||
        poll.correct_option_id < 0 ||
        ...
        errors.push(`Неверный правильный ответ итогового Poll ${index + 1}`);
      }
    });
  }
} catch (error) {
...
console.log("✓ Настроены 30 ежедневных Telegram Poll");
console.log("✓ Проверен итоговый опросник месяца");
