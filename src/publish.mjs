...
const SEASONS = {
  1: {
    ...
    congratsImage: "images/season1-congratulations.jpg",
    congratsText: `<b>🎉 Поздравляем!</b>
...
До встречи во втором сезоне! 🇪🇸`
  ...
  2: {
    lessonsFile: "data/lessons-season2.json",
    congratsImage: "images/season2/season2-congratulations.jpg",
    congratsText: `<b>🎉 ¡Felicidades! Сезон 2 завершён.</b>

За этот месяц вы научились общаться в кафе, называть дни недели, договариваться о встрече, уточнять время и место, менять планы и принимать приглашение.

История продолжается: впереди Мадрид — улицы, транспорт и новые маршруты. 🇪🇸`
  },
  3: {
    lessonsFile: "data/lessons-season3.json",
    congratsImage: "images/season3/season3-congratulations.jpg",
    congratsText: `<b>🎉 ¡Felicidades! Сезон 3 завершён.</b>

За этот месяц вы научились ориентироваться в городе, спрашивать дорогу, понимать простые указания, пользоваться базовой лексикой метро и автобуса и спокойно исправлять ошибку маршрута.

История продолжается: впереди новые люди, друзья Адриана, разговоры о характере и увлечениях — Сезон 4 «Amigos». 🇪🇸`
  }
};
...
function seasonNumberForMonth(startDate, now) {
...
async function publishLessonSlot(context) {
  ...
  if (!alreadyPublished(context, audioKey)) {
    await sendFile("sendAudio", channel, "audio", lesson.audio, {
      title: slot === "morning" ? `Сезон ${context.seasonNumber} • День ${day} • Утро` : `Сезон ${context.seasonNumber} • День ${day} • Практика`,
...
async function publishCongrats(context) {
  ...
  await sendFile("sendPhoto", context.channel, "photo", context.seasonConfig.congratsImage, {
    caption: context.seasonConfig.congratsText,
    parse_mode: "HTML"
  });
  markPublished(context, key);
}
...
async function main() {
  ...
  const manualSeason = Number(process.env.MANUAL_SEASON || 0);
  ...
  const automaticSeasonNumber = seasonNumberForMonth(startDate, now);
  const seasonNumber = eventType === "manual" && manualSeason > 0 ? manualSeason : automaticSeasonNumber;
  const seasonConfig = SEASONS[seasonNumber];
  if (!seasonConfig) {
    console.log(`Сезон ${seasonNumber} пока не настроен. Публикация не требуется.`);
    return;
  }

  const lessons = readJson(path.join(ROOT, seasonConfig.lessonsFile));
  if (!Array.isArray(lessons) || lessons.length < 28 || lessons.length > 31) {
    throw new Error(`${seasonConfig.lessonsFile} должен содержать от 28 до 31 дня`);
  }
  ...
  console.log(`Активный сезон: ${seasonNumber}. Файл: ${seasonConfig.lessonsFile}`);
